...
String search_name = GlobalVariable.account_merchant_name

List compare_names = [[By.cssSelector(GlobalVariable.account_merchant_ele)], '其他查询',
	 By.className(GlobalVariable.empty_tips), [true]]  //  [index=3]-字段是否在列表存在(boolean),空值和逆向不用传]

WebUI.click(findTestObject('系统管理/账号管理/名称搜索框'))
...
while(true) {
	...
	List<WebElement> click_open = driver.findElements(By.cssSelector(GlobalVariable.account_open_button))
...
List<WebElement> result = driver.findElements(By.cssSelector(GlobalVariable.account_merchant_box_value))
