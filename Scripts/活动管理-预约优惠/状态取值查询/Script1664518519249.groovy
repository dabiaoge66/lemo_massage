...
String search_name = GlobalVariable.discount_status

List compare_names = [[By.cssSelector(GlobalVariable.tech_search_ele)], '其他查询',
	 By.className(GlobalVariable.empty_tips), [true]]  //  [index=3]-字段是否在列表存在(boolean),空值和逆向不用传]

WebUI.click(findTestObject('活动管理/预约优惠/状态搜索框'))

List<WebElement> result = driver.findElements(By.cssSelector(GlobalVariable.discount_box_data))
...
WebUI.click(findTestObject('活动管理/预约优惠/快速查询按钮'))

WebUI.delay(1)

WebUI.switchToFrame(findTestObject('frames/主页面/活动管理-预约优惠'), 30)
...
void setUp() {
	...
	WebUI.click(findTestObject('活动管理/导航栏/预约优惠'))
}
