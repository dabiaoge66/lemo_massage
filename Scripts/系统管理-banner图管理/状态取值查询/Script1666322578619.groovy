...
String search_name = GlobalVariable.banner_status

List compare_names = [[By.cssSelector(GlobalVariable.banner_status_ele)], '其他查询',
	 By.className(GlobalVariable.empty_tips), [true]]  //  [index=3]-字段是否在列表存在(boolean),空值和逆向不用传]

WebUI.click(findTestObject('系统管理/banner图管理/状态搜索框'))

List<WebElement> result = driver.findElements(By.cssSelector(GlobalVariable.accounts_member_level_value))
...
WebUI.click(findTestObject('系统管理/banner图管理/快速查询按钮'))

WebUI.delay(1)

WebUI.switchToFrame(findTestObject('frames/主页面/系统管理-banner图管理'), 30)
...
void setUp() {
	...
	WebUI.click(findTestObject('系统管理/导航栏/banner图管理'))
}
