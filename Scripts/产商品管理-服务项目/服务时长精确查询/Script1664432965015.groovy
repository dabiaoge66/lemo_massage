...
String search_name = GlobalVariable.exact_service_time

List compare_names = [[By.cssSelector(GlobalVariable.service_times_ele)],'精确查询',
	By.className(GlobalVariable.empty_tips), [true]]  //  [index=3]-字段是否在列表存在(boolean),空值和逆向不用传]

WebUI.sendKeys(findTestObject('产商品管理/服务项目/服务时长-搜索框'), search_name)

WebUI.click(findTestObject('产商品管理/服务项目/快速查询按钮'))

WebUI.delay(1)

WebUI.switchToFrame(findTestObject('frames/主页面/产商品管理-服务项目'), 30)
...
void setUp() {
    ...
    WebUI.click(findTestObject('产商品管理/导航栏/服务项目'))
}
