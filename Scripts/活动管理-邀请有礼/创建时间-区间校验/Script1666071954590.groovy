...
String search_data = GlobalVariable.create_invite_start + '/' + GlobalVariable.create_invite_end

String[] search_name = search_data.split('/')

List compare_names = [[By.cssSelector(GlobalVariable.invite_created_time)], '日期比较',
	 By.className(GlobalVariable.empty_tips), [true]]  //  [index=3]-字段是否在列表存在(boolean)

WebUI.sendKeys(findTestObject('活动管理/邀请有礼/创建时间-开始'), search_name[0])

WebUI.sendKeys(findTestObject('活动管理/邀请有礼/创建时间-结束'), search_name[1])

WebUI.click(findTestObject('活动管理/邀请有礼/快速查询按钮'))

WebUI.delay(1)

WebUI.switchToFrame(findTestObject('frames/主页面/活动管理-邀请有礼'), 30)
...
void setUp() {
	...
	WebUI.click(findTestObject('活动管理/导航栏/邀请有礼'))
}
