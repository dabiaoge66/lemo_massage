...
String search_name = GlobalVariable.invite_status

List compare_names = [[By.cssSelector(GlobalVariable.invite_state_ele)], '其他查询',
	 By.className(GlobalVariable.empty_tips), [true]]  //  [index=3]-字段是否在列表存在(boolean),空值和逆向不用传]

WebUI.click(findTestObject('活动管理/邀请有礼/状态搜索框'))

List<WebElement> result = driver.findElements(By.cssSelector(GlobalVariable.invite_status_value))
