...
String search_name = GlobalVariable.select_store_name

List compare_names = [[By.cssSelector(GlobalVariable.schedule_sids_ele)], '其他查询',
	 By.className(GlobalVariable.empty_tips), [true]]  //  [index=3]-字段是否在列表存在(boolean)

WebUI.sendKeys(findTestObject('产商品管理/排班管理/门店名称搜索框'), search_name)

WebUI.click(findTestObject('产商品管理/排班管理/门店名称搜索框'))

List<WebElement> search_list = driver.findElements(By.cssSelector(GlobalVariable.sid_box_values))
