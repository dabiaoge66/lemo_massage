...
String search_data = GlobalVariable.schedule_begin_time + '/' + GlobalVariable.schedule_end_time

String[] search_name = search_data.split('/')

List compare_names = [[By.cssSelector(GlobalVariable.schedule_start_date)],
	 '日期比较', By.className(GlobalVariable.empty_tips), [true]]  //  [index=3]-字段是否在列表存在(boolean)

WebUI.sendKeys(findTestObject('产商品管理/排班管理/开始日期-开始时间'), search_name[0])

WebUI.sendKeys(findTestObject('产商品管理/排班管理/开始日期-结束时间'), search_name[1])
