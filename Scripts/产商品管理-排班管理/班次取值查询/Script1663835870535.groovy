...
String search_name = GlobalVariable.shift_search_data

List compare_names = [[By.cssSelector(GlobalVariable.schedule_shifts_ele)], '其他查询',
	 By.className(GlobalVariable.empty_tips), [true]]  //  [index=3]-字段是否在列表存在(boolean),空值和逆向不用传]

WebUI.sendKeys(findTestObject('产商品管理/排班管理/班次搜索框'), search_name)

WebUI.click(findTestObject('产商品管理/排班管理/班次搜索框'))

List<WebElement> search_list = driver.findElements(By.cssSelector(GlobalVariable.shift_box_values))
