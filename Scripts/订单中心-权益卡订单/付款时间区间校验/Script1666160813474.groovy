...
String search_data = GlobalVariable.rights_pay_start + '/' + GlobalVariable.rights_pay_end

String[] search_name = search_data.split('/')

List compare_names = [[By.cssSelector(GlobalVariable.rights_pay_time_ele)], '日期比较',
	 By.className(GlobalVariable.empty_tips), [true]]  //  [index=3]-字段是否在列表存在(boolean)

WebUI.sendKeys(findTestObject('订单中心/权益卡订单/付款时间-开始'), search_name[0])

WebUI.sendKeys(findTestObject('订单中心/权益卡订单/付款时间-结束'), search_name[1])

WebUI.click(findTestObject('订单中心/权益卡订单/快速查询按钮'))

WebUI.delay(1)

WebUI.switchToFrame(findTestObject('frames/主页面/订单中心-权益卡订单'), 30)
...
void setUp() {
	...
	WebUI.click(findTestObject('订单中心/导航栏/权益卡订单'))
}
