...
String search_data = GlobalVariable.arrival_store_start + '/' + GlobalVariable.arrival_store_end

String[] search_name = search_data.split('/')

List compare_names = [[By.cssSelector(GlobalVariable.arrival_store_time_ele)], '日期比较',
	 By.className(GlobalVariable.empty_tips), [true]]  //  [index=3]-字段是否在列表存在(boolean)

WebUI.sendKeys(findTestObject('订单中心/服务订单/到店时间-开始'), search_name[0])

WebUI.sendKeys(findTestObject('订单中心/服务订单/到店时间-结束'), search_name[1])
