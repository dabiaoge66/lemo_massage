...
String search_data = GlobalVariable.service_order_start + '/' + GlobalVariable.service_order_end

String[] search_name = search_data.split('/')

List compare_names = [[By.cssSelector(GlobalVariable.service_order_time)], '日期比较',
	 By.className(GlobalVariable.empty_tips), [true]]  //  [index=3]-字段是否在列表存在(boolean)

WebUI.sendKeys(findTestObject('订单中心/服务订单/下单时间-开始'), search_name[0])

WebUI.sendKeys(findTestObject('订单中心/服务订单/下单时间-结束'), search_name[1])
