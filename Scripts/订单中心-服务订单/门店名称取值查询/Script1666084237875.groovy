...
String search_name = GlobalVariable.service_store_name

List compare_names = [[By.cssSelector(GlobalVariable.service_orderNo_ele)], '其他查询',
	 By.className(GlobalVariable.empty_tips), [false]]  //  [index=3]-字段是否在列表存在(boolean),空值和逆向不用传]

WebUI.click(findTestObject('订单中心/服务订单/门店名称搜索框'))

List<WebElement> result = driver.findElements(By.cssSelector(GlobalVariable.service_box_value))
...
WebUI.click(findTestObject('订单中心/服务订单/快速查询按钮'))

WebUI.delay(1)

WebUI.switchToFrame(findTestObject('frames/主页面/订单中心-服务订单'), 30)
...
void setUp() {
	...
	WebUI.click(findTestObject('订单中心/导航栏/服务订单'))
}
