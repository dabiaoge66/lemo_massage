...
String search_name = GlobalVariable.card_status

List compare_names = [[By.cssSelector(GlobalVariable.card_state_ele)], '其他查询',
	 By.className(GlobalVariable.empty_tips), [true]]  //  [index=3]-字段是否在列表存在(boolean),空值和逆向不用传]

WebUI.click(findTestObject('活动管理/权益卡管理/状态搜索框'))

List<WebElement> result = driver.findElements(By.cssSelector(GlobalVariable.card_status_value))
