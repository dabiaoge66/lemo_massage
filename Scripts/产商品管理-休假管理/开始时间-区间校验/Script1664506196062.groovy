...
String search_data = GlobalVariable.vacation_start_time + '/' + GlobalVariable.vacation_end_time

String[] search_name = search_data.split('/')

List compare_names = [[By.cssSelector(GlobalVariable.vacation_begin_time)], '日期比较',
	 By.className(GlobalVariable.empty_tips), [true]]  //  [index=3]-字段是否在列表存在(boolean)

WebUI.sendKeys(findTestObject('产商品管理/休假管理/开始时间-开始'), search_name[0])

WebUI.sendKeys(findTestObject('产商品管理/休假管理/开始时间-结束'), search_name[1])
