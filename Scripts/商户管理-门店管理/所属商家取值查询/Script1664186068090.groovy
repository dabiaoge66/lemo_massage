...
String search_name = GlobalVariable.select_merchant_name

List compare_names = [[By.cssSelector(GlobalVariable.store_names_ele)], '其他查询',
	 By.className(GlobalVariable.empty_tips), [false]]  //  [index=3]-字段是否在列表存在(boolean),空值和逆向不用传]

WebUI.sendKeys(findTestObject('商户管理/门店管理/所属商家搜索框'), search_name)

WebUI.click(findTestObject('商户管理/门店管理/所属商家搜索框'))

List<WebElement> search_list = driver.findElements(By.cssSelector(GlobalVariable.merchant_box_values))
...
WebUI.click(findTestObject('商户管理/门店管理/快速查询按钮'))

WebUI.delay(1)

WebUI.switchToFrame(findTestObject('frames/主页面/商户管理-门店管理'), 30)
