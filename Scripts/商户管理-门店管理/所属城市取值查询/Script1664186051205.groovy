...
String search_name = GlobalVariable.select_city_name

List compare_names = [[By.cssSelector(GlobalVariable.store_citys_ele)], '其他查询',
	 By.className(GlobalVariable.empty_tips), [true]]  //  [index=3]-字段是否在列表存在(boolean),空值和逆向不用传]

WebUI.sendKeys(findTestObject('商户管理/门店管理/所属城市搜索框'), search_name)

WebUI.click(findTestObject('商户管理/门店管理/所属城市搜索框'))

List<WebElement> search_list = driver.findElements(By.cssSelector(GlobalVariable.city_box_values))
