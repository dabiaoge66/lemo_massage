...
String search_name = GlobalVariable.role_merchant_name

List compare_names = [[By.cssSelector(GlobalVariable.role_merchant_ele)], '其他查询',
	 By.className(GlobalVariable.empty_tips), [true]]  //  [index=3]-字段是否在列表存在(boolean),空值和逆向不用传]

WebUI.click(findTestObject('系统管理/角色管理/所属商户搜索框'))
...
while(true) {
	...
	List<WebElement> click_open = driver.findElements(By.cssSelector(GlobalVariable.role_open_button))
	
	if(click_open.size() == 0) {
		break out
	}
	
	for (int a=0; a<click_open.size(); a++) {
		click_open[a].click()
	}
}

List<WebElement> result = driver.findElements(By.cssSelector(GlobalVariable.role_merchant_value))
...
WebUI.click(findTestObject('系统管理/角色管理/快速查询按钮'))

WebUI.delay(1)

WebUI.switchToFrame(findTestObject('frames/主页面/系统管理-角色管理'), 30)
...
void setUp() {
	...
	WebUI.click(findTestObject('系统管理/导航栏/角色管理'))
}
