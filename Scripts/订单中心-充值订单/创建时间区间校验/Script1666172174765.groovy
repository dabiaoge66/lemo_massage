...
String search_data = GlobalVariable.recharge_create_start + '/' + GlobalVariable.recharge_create_end

String[] search_name = search_data.split('/')

List compare_names = [[By.cssSelector(GlobalVariable.recharge_create_time)], '日期比较',
	 By.className(GlobalVariable.empty_tips), [true]]  //  [index=3]-字段是否在列表存在(boolean)

WebUI.sendKeys(findTestObject('订单中心/充值订单/创建时间-开始'), search_name[0])

WebUI.sendKeys(findTestObject('订单中心/充值订单/创建时间-结束'), search_name[1])
