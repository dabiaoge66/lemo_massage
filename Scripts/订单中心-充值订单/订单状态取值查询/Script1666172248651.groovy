...
String search_name = GlobalVariable.recharge_order_status

List compare_names = [[By.cssSelector(GlobalVariable.recharge_status_ele)], '其他查询',
	 By.className(GlobalVariable.empty_tips), [true]]  //  [index=3]-字段是否在列表存在(boolean),空值和逆向不用传]

WebUI.click(findTestObject('订单中心/充值订单/订单状态搜索框'))

List<WebElement> result = driver.findElements(By.cssSelector(GlobalVariable.recharge_box_value))
