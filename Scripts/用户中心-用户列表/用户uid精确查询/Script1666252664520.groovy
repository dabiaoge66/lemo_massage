...
String search_name = GlobalVariable.accounts_exact_uid

List compare_names = [[By.cssSelector(GlobalVariable.accounts_uid_ele)],'精确查询',
	By.className(GlobalVariable.empty_tips), [true]]  //  [index=3]-字段是否在列表存在(boolean),空值和逆向不用传]

WebUI.sendKeys(findTestObject('用户中心/用户列表/用户UID搜索框'), search_name)
